...
from booking.models import (
    ...
    NewQuoteServiceBookDetail, NewQuoteServiceBookDetailAllotment,
    NewQuoteServiceBookDetailTransfer, NewQuoteServiceBookDetailExtra,
...
class BookingServices(object):
    ...
    def build_bookingservice_from_quoteservice(
        ...
        if service_pax_variant:
            ...
            cls.setup_bookingservice_amounts(booking_service, pax_list, booking.booked)
            ...
            with transaction.atomic(savepoint=False):
                ...
                for quote_package in QuoteExtraPackage.objects.filter(quote_id=quote.id).all():
                    ...
                    if service_pax_variant:
                        ...
                        cls.setup_bookingservice_amounts(booking_package, pax_list, booking.booked)
        ...
        if allotment_list:
            ...
            for allotment in allotment_list:
                ...
                if not hasattr(allotment, 'service'):
                    ...
                    c2, c2_msg, p2, p2_msg, \
                    c3, c3_msg, p3, p3_msg, \
                    c4, c4_msg, p4, p4_msg = cls._find_quoteservice_amounts(
    ...
    def _find_quoteservice_amounts(
        ...
        quoteservice_pax_variant = cls._find_service_pax_variant(
        ...
        c1, c1_msg, c2, c2_msg, c3, c3_msg, c4, c4_msg = cls._find_quoteservice_costs(
            ...
            quoteservice_pax_variant, manuals)

        p1, p1_msg, p2, p2_msg, p3, p3_msg, p4, p4_msg = cls._find_quoteservice_prices(
            pax_quantity, quoteservice, date_from, date_to, agency,
            quoteservice_pax_variant, manuals, quote_pax_variant, c1, c2, c3, c4)
    ...
    def _find_quoteservice_catalog_costs(
        ...
        if grouping:
            ...
            c1, c1_msg = cls._any_quoteservice_catalog_costs(
                quoteservice, date_from, date_to, ({0:1, 1:0},), provider)
            c2, c2_msg = cls._any_quoteservice_catalog_costs(
                quoteservice, date_from, date_to, ({0:2, 1:0},), provider)
            c3, c3_msg = cls._any_quoteservice_catalog_costs(
                quoteservice, date_from, date_to, ({0:3, 1:0},), provider)
            c4, c4_msg = cls._any_quoteservice_catalog_costs(
    ...
    def _find_quoteservice_details_costs(
        ...
        if hasattr(quoteservice, 'id') and quoteservice.id:
            ...
            allotment_list = list(
                ServiceBookDetailAllotment.objects.filter(service=quoteservice.service_id).all())
            transfer_list = list(
                ServiceBookDetailTransfer.objects.filter(service=quoteservice.service_id).all())
            extra_list = list(
                ServiceBookDetailExtra.objects.filter(service=quoteservice.service_id).all())
    ...
    def find_booking_amounts(cls, booking, pax_list):
        ...
        if not service_list and not package_list:
        ...
        if service_list:
            for service in service_list:
                ...
                if service.booking_package is None:
                    c, c_msg, p, p_msg = cls._find_bookingservice_update_amounts(
                        bookingservice=service, agency=agency)
                    if c is None:
                        c_msg = "%s - %s" % (service, c_msg)
                    if p is None:
                        p_msg = "%s - %s" % (service, p_msg)
    ...
    def find_bookingservice_amounts(
            cls, bookingservice, pax_list=None):
        if isinstance(bookingservice, BookingExtraPackage):
            return cls._find_bookingpackage_update_amounts(
    ...
    def _find_bookingservice_cost(cls, bookingservice, pax_list=None, booked=None):
        if isinstance(bookingservice, BookingExtraPackage):
            cost, cost_msg, price, price_msg = cls._find_bookingpackage_update_amounts(
                bookingservice, pax_list)
        ...
        if not hasattr(bookingservice, "cost_by_catalog"):
            ...
            if not booked:
                booked = bookingservicedetail.booking_service.booking.booked
    ...
    def _find_bookingservice_price(cls, bookingservice, pax_list=None, agency=None, booked=None):
        ...
        if not booked:
            if hasattr(bookingservice, "booking"):
                ...
                # is detail
                booked = bookingservice.booking_service.booking.booked
    ...
    def _find_bookingservice_catalog_cost(
            cls, bookingservice, date_from, date_to, pax_list, provider, booked, contract_code):

        service = None

        if isinstance(bookingservice, (BookingProvidedService)):
            service = bookingservice.service
        elif isinstance(bookingservice, (BookingBookDetail, ServiceBookDetail)):
            service = bookingservice.book_service

        cost_groups = BookingServices.find_bookingservice_paxes_groups(
            pax_list, service, True)

        if isinstance(bookingservice, (
                BookingProvidedAllotment, BookingBookDetailAllotment, ServiceBookDetailAllotment)):
            cost, cost_msg = ConfigServices.allotment_costs(
                service,
                date_from, date_to,
                cost_groups, provider, booked, contract_code,
                bookingservice.board_type, bookingservice.room_type_id,
                bookingservice.service_addon_id)
        elif isinstance(bookingservice, (
                BookingProvidedTransfer, BookingBookDetailTransfer, ServiceBookDetailTransfer)):
            cost, cost_msg = ConfigServices.transfer_costs(
                ...
                BookingProvidedExtra, BookingBookDetailExtra, ServiceBookDetailExtra)):
    ...
    def _find_bookingservice_catalog_price(
        ...
        if isinstance(bookingservice, (
            ...
            price, price_msg = ConfigServices.allotment_prices(
                service,
                date_from, date_to,
                price_groups, agency, booked, contract_code,
                bookingservice.board_type, bookingservice.room_type_id,
                bookingservice.service_addon_id)
        elif isinstance(bookingservice, (
                BookingProvidedTransfer, BookingBookDetailTransfer, ServiceBookDetailTransfer)):
    ...
    def _find_bookingservice_amounts_by_details(
            cls, bookingservice, pax_list, agency, booked):
        ...
        if hasattr(bookingservice, 'id') and bookingservice.id:
            ...
            allotment_list = list(BookingBookDetailAllotment.objects.filter(
                booking_service=bookingservice.id).all())
            transfer_list = list(BookingBookDetailTransfer.objects.filter(
                booking_service=bookingservice.id).all())
            extra_list = list(BookingBookDetailExtra.objects.filter(
                booking_service=bookingservice.id).all())
        else:
            # details from config service details
            allotment_list = list(ServiceBookDetailAllotment.objects.filter(
                service=bookingservice.service_id).all())
            transfer_list = list(ServiceBookDetailTransfer.objects.filter(
                service=bookingservice.service_id).all())
            extra_list = list(ServiceBookDetailExtra.objects.filter(
                service=bookingservice.service_id).all())
            ...
            for bookingservicedetail in service_list:
                ...
                if not cost_msg: # is None: here, cost_msg can be null or empty string
                    ...
                    if bookingservicedetail.manual_cost:
                        ...
                        c, c_msg = cls._find_bookingservice_cost(
                            bookingservicedetail, pax_list, booked)
                    cost, cost_msg = cls._merge_costs(cost, cost_msg, c, c_msg)
                ...
                if not price_msg:  # is None:  here, price_msg can be null or empty string
                    ...
                    if bookingservicedetail.manual_price:
                        ...
                        p, p_msg = cls._find_bookingservice_price(
                            bookingservicedetail, pax_list, agency, booked)
                    price, price_msg = cls._merge_prices(price, price_msg, p, p_msg)
    ...
    def _find_bookingservice_update_amounts(
        ...
        if isinstance(bookingservice, BookingExtraPackage):
            return cls._find_bookingpackage_update_amounts(
                bookingservice, pax_list, agency, booked, True)

        cost, cost_msg = cls._find_bookingservice_update_cost(bookingservice, pax_list)
        price, price_msg = cls._find_bookingservice_update_price(
            bookingservice, pax_list, agency, booked)
    ...
    def _find_bookingservice_update_price(cls, bookingservice, pax_list=None, agency=None, booked=None):
        ...
        if isinstance(bookingservice, BookingExtraPackage):
            cost, cost_msg, price, price_msg = cls._find_bookingpackage_update_amounts(
                bookingpackage, pax_list, agency, booked)
            return price, price_msg
    ...
    def setup_bookingservice_amounts(cls, bookingservice, pax_list=None, agency=None, booked=None):
        ...
        if not bookingservice.manual_cost:
            if not bookingservice.manual_price:
                ...
                cost, cost_msg = cls._find_bookingservice_cost(
                    bookingservice, pax_list, booked)
                if not cls._equals_amounts(bookingservice.cost_amount, cost):
                    modified = True
                    bookingservice.cost_amount = cost
        else:
            price, price_msg = cls._find_bookingservice_price(
                bookingservice, pax_list, agency, booked)
            if not cls._equals_amounts(bookingservice.price_amount, price):
                modified = True
                bookingservice.price_amount = price
    ...
    def _find_bookingpackage_update_amounts(
            cls, bookingservice, pax_list=None, agency=None, booked=None, manuals=False):
        if isinstance(bookingservice, BookingExtraPackage):
            bookingpackage = bookingservice
        else:
            bookingpackage = bookingservice.booking_package
        ...
        if agency and bookingpackage.price_by_catalog:
            ...
            for service in service_list:
                c, c_msg = cls._find_bookingservice_update_cost(service)
                cost, cost_msg = cls._merge_costs(cost, cost_msg, c, c_msg)
                if cost is None:
                    return cost, cost_msg, price, price_msg
        else:
            for service in service_list:
                c, c_msg, p, p_msg = cls._find_bookingservice_update_amounts(service)
                cost, cost_msg = cls._merge_costs(cost, cost_msg, c, c_msg)
                price, price_msg = cls._merge_prices(price, price_msg, p, p_msg)
                if cost is None and price is None:
                    return cost, cost_msg, price, price_msg
    ...
    def update_booking(cls, booking_or_bookingservice):
        ...
        date_from = None
        date_to = None
        # date to use when all services are cancelled
        date_from_min = None
        # date to use when all services are cancelled
        date_to_max = None
        status = constants.BOOKING_STATUS_NOSHOW
        booking_with_services = False
        all_services_cancelled = True
        for service in BaseBookingService.objects.filter(booking=booking):
            booking_with_services = True
            if all_services_cancelled:
                # update date_from_min and date_to_max
                if (service.datetime_from is not None
                        and (date_from_min is None or date_from_min > service.datetime_from)):
                    date_from_min = service.datetime_from
                if service.datetime_to is None:
            ...
            if service.status in [constants.SERVICE_STATUS_CANCELLED, constants.SERVICE_STATUS_CANCELLING]:
                # cancelled services not processed
                continue
            # verify service is detail
            is_detail = False
            if service.base_category in [
                    constants.BASE_BOOKING_SERVICE_CATEGORY_BOOKING_DETAIL_ALLOTMENT,
                    constants.BASE_BOOKING_SERVICE_CATEGORY_BOOKING_DETAIL_TRANSFER,
                    constants.BASE_BOOKING_SERVICE_CATEGORY_BOOKING_DETAIL_EXTRA]:
                is_detail = True
                # load detail
                detail = BookingBookDetail.objects.get(pk=service.id)
                # verify parent is cancelled
                if detail.booking_service.status in [constants.SERVICE_STATUS_CANCELLED, constants.SERVICE_STATUS_CANCELLING]:
                    # detail service with parent cancelled not processed
                    continue
            ...
            # verify service is detail
            if is_detail:
                # details are not used for status
                continue

            # set not all cancelled
            all_services_cancelled = False
    ...
    def _clone_booking_service(cls, booking_service, booking_package, booking):
        ...
        if booking_package:
            booking_service.booking_package = booking_package
            booking_service.booking_package_id = booking_package.pk
        booking_service.booking = booking
    ...
    def clone_booking_services(cls, old_booking_id, new_booking):
        ...
        allotments = list(BookingProvidedAllotment.objects.filter(
            booking=old_booking, booking_package__isnull=True))
        for service in allotments:
            cls._clone_booking_service(service, None, new_booking)
    ...
    def _clone_booking_package(cls, booking_package_service, booking):
        allotments = list(BookingProvidedAllotment.objects.filter(
            booking_package=booking_package_service))
        transfers = list(BookingProvidedTransfer.objects.filter(
            booking_package=booking_package_service))
        extras = list(BookingProvidedExtra.objects.filter(
            booking_package=booking_package_service))

        booking_package_service.pk = None
        booking_package_service.id = None
        booking_package_service.basebookingservice_ptr = None
        booking_package_service.basebookingservice_ptr_id = None
        booking_package_service.bookingservice_ptr = None
        booking_package_service.bookingservice_ptr_id = None
        booking_package_service.booking = booking
        booking_package_service.booking_id = booking.pk
        booking_package_service.status = constants.SERVICE_STATUS_PENDING
        booking_package_service.conf_number = ''
        if not hasattr(booking_package_service, 'manual_cost') or not booking_package_service.manual_cost:
            booking_package_service.cost_amount = None
        if not hasattr(booking_package_service, 'manual_price') or not booking_package_service.manual_price:
            booking_package_service.price_amount = None
